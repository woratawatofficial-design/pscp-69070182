...
```text
รับ input ไพ่ อะไรก็ได้ เช่น Q10 หรือ Q หรือ QS
output print เป็น input ตัวเเรก of ตัวที่สอง
เช่น input = Q10 = queen of 10
...
```text
สร้าง dictionary เก็บไพ่ที่เป็นตัวอักษร
รับค่า input.lower()
สร้างตัวเเปร Y เเล้วใช้ func len() เพื่อนำค่า Y ไปเข้าเงื่อนไข 
หาก y = 2 
จะเเยก input เป็น [0] เเละ [1] จากนั้นเข้าเงื่อนไขif else
เเล้ว print N[0] of N[1]
หาก y = 1 ใช้ condition หาก  N[0] อยู่ใน dictionary card จ print Caards[N]
หากไม่อยู่ จprint N
หาก Y = 3
จะใช้ condition เช็ค N[0] หากอยู่ใน card จะprint CARDS[N] of N[1]N[2]
หาก N[2 อยู่ใน] Card จะprint N[0]N[1] of CARDS[N]
```
...
```text
คล้ายเเผนเเรก เเต่เพิ่มเติมเงื่อนไขเพื่อให้รองรับ testcase
```
...
```text
เพื่อตรวจสอบหาก ตัวอักษรไม่อยู่ใน dictionary cards
```
