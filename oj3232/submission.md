...
```text
รับ input 2ค่าใน 1 บรรทัด X,Y โดย Xระยะที่เริ่ม Y คือจุดที่หยุด
Output : จำนวนครั้งที่กบต้องกระโดด หาก x เป็น 0 เเล้วยังไม่ถึง y จะprint 0
constraint : กบกระโดดได้ลดลงจาก X ครั้งละ -2 เมตร
...
```text
รับค่า input เเล้ว split
สร้างตัวเเปร count
ใช้ while loop หาก X มากกว่า 0
total += X
X -= 2เรื่อยๆ
count +=1 
หาก X น้อยกว่าหรือเท่ากับ 0
จะเช็คเงื่อนไข
ถ้า total มากกว่าหรือเท่ากับ y จะprint count
ถ้า total ไม่ถึง y หรือเป้าหมาย print -1
```
...
```text
เหมือนกับเเผนเเรก เเต่เปลี่ยนเงื่อนไข หาก total ไม่ถึง y มาไว้นอก while loop เพราะจะ breakตอน total >= Y
```
