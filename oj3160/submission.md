...
```text
รับค่า X,Y
output คือหาจำนวนเฉพาะจากช่วง X ถึง Y (เอาYด้วย)
printว่ามีตัวอะไรบ้างเเละมีทั้งหมดกี่ตัง
```
...
```text
รับค่า input X,Y จากนั้นใช้ for i in range(X,Y+1) จะได้ i คือจำนวนตั้งเเต่ X ถึง Y
สร้างlist เเละตัวเเปร count เพื่อนับ
จากนั้นจะเอาเเต่ละตัวของ i ไปเช็คว่าหารตัวอะไรลงตัวบ้าง ถ้าหารได้ลงตัวจะถือว่าไม่เป็นจำนวนเฉพาะ ถ้าไม่มี ก็จะ append ไปที่list เเละ count += 1 เเละ print
```
...
```text
สร้าง list เก็บ จำนวนเฉพาะ
สร้างตัวเเปร count
รับค่า x,y
ใช้ for i in range(x,y+1)
จากนั้นกำหนดไว้ก่อนว่าPrime = True
เเละสร้างเงื่อนไขถ้า iน้อยกว่า2จะไม่ถือเป็น prime ดังนั้น
prime = false
เเละใช้ elseเพื่อเช็คที่เหลือ
โดยสร้าง for j in range(2,i) เนื่องจากจำนวนเฉพาะคือจำนวนที่หารได้เเค่ 1กับตัวมันเอง จึงต้องหาร 2ถึงก่อนตัวมัน สร้างเงื่อนไข หาก i % j ได้ 0 เเปลว่าหารลงตัว prime = false
สร้างเงื่อนไขต่อ หากprime เป็น true หรือเป็นจำนวนเฉพาะ จะappend i เข้าlist เเละ count +=1
เเละถ้ามี prime number จะprint count เเละprint total prime
หากไม่มี count หรือ count = 0 จะ printเเค่ Total primes
```
...
```text
หากเป็น 1 เเละ 2 จะนับ 2หรือไม่
```
