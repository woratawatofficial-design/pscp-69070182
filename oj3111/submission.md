...
```text
รับ input ตัวอักษร Y/N
รับ input จำนวนสินค้าที่ซื้อ
รับ input ราคาของสินค้าตามจำนวนสินต้าที่ซื้อ
เเสดงยอดหลังหักส่วนลดโดย
constraint : หากเป็นmember หรือ Y จะได้รับส่วนลด 5% จาก total เเละถ้าหากไม่ใช่ member เเต่ยอดเกิน 500บาท จะได้รับส่วนลด 3% จาก total
กรณีอื่นไม่ลดราคา
print output ทศนิยมสองตำเเหน่ง
...
```text
สร้างตัวเเปร Member รับค่า str input (Y or N)
สร้างตัวเเปร QTT รับค่าจำนวนสินค้าที่ซื้อ
สร้างตัวเเปร total สำหรับเก็บยอดที่ซื้อ
ใช้ for loop, for _ in QTT คือจะให้ input ราคาสินค้าเเต่ละชิ้นตามจำนวนที่ซื้อทั้งหมด
เเละจากนั้นจะทำค่าไปเพิ่มใน total
ใช้ condition สร้าง เงื่อนไขหากเป็น member , total -= total*0.05 ก็คือรับส่วนลด 5%
หากไม่ใช่ member เเต่ยอด total เกิน 500, total -= total*0.03 ได้รับส่วนลด 3%
print totalทศนิยม 2ตำเเหน่ง
```
...
```text
คล้ายกับเเผนเเรกเเต่เพิ่มก่อน print มาคือได้นำ total มาคูณ 100 เเละหาร 100 เเละใช้ math.ceil มาใช้ปัด total ทศนิยมตำเเหน่งที่ 3 จากนั้นใช้ f string print totalทศนิยม 2ตำเเหน่ง
```
...
```text
หาก input สถานีไม่ตรงกับเงื่อนไข จะเเสดงผลอย่างไร
```
...
```text
หาก input ตรงกับเงื่อนไข จะเเสดงผลถูกหรือไม่
```
...
```text
หาก input สถานีเดิม จะเเสดงผลอย่างไร
```
